...
function Navbar() {
  ...
  // Re-read sessionStorage every time the location changes.
  useEffect(() => {
    const storedFullName = sessionStorage.getItem("fullName"); // Updated to sessionStorage
    if (storedFullName) {
  ...
  const handleLogout = () => {
    sessionStorage.removeItem("token");   // Updated to sessionStorage
    sessionStorage.removeItem("uid");     // Updated to sessionStorage
    sessionStorage.removeItem("fullName"); // Updated to sessionStorage
    setUserFullName(null);
    navigate("/");
  };
  ...
  <div className="user-info">
    <div className="user-initials">
      {userFullName
        .split(" ")
        .map((word, index) => (index === 0 || index === 1 ? word[0] : ""))
        .join("")
        .toUpperCase()}
    </div>
